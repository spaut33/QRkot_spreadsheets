...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def get(self, obj_id, session: AsyncSession) -> Optional[ModelType]:
    ...
    async def update(
        self, db_obj, session: AsyncSession, obj_in=None
    ) -> ModelType:
        """Обновление объекта."""
        if obj_in:
    ...
    async def get_not_closed(self, session: AsyncSession) -> List[ModelType]:
        """Получение всех не закрытых проектов."""
